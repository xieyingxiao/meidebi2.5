...
public class SchemaGenerator {
	...
	public static void createDatabase(SQLiteDatabase sqLiteDatabase) {
		List<Class> domainClasses = getDomainClasses(XApplication.getInstance());
		for (Class domain : domainClasses) {
			createTable(domain, sqLiteDatabase);
		}
	}

	public void doUpgrade(SQLiteDatabase sqLiteDatabase, int oldVersion,
			int newVersion) {
		List<Class> domainClasses = getDomainClasses(context);
		for (Class domain : domainClasses) {
			try { // we try to do a select, if fails then (?) there isn't the
					// table
				sqLiteDatabase.query(NamingHelper.toSQLName(domain), null,
						null, null, null, null, null);
			} catch (SQLiteException e) {
				Log.i("Sugar", String.format(
						"Creating table on update (error was '%s')",
						e.getMessage()));
				createTable(domain, sqLiteDatabase);
			}
		}
	...
	public void deleteTables(SQLiteDatabase sqLiteDatabase) {
		...
		for (Class table : tables) {
			sqLiteDatabase.execSQL("DROP TABLE IF EXISTS "
					+ NamingHelper.toSQLName(table));
		}
	}

	private boolean executeSugarUpgrade(SQLiteDatabase db, int oldVersion,
			int newVersion) {
		boolean isSuccess = false;

		try {
			List<String> files = Arrays.asList(this.context.getAssets().list(
					"sugar_upgrades"));
	...
	private static void createTable(Class<?> table,
		...
		for (Field column : fields) {
			...
			if (columnType != null) {
				...
				if (column.isAnnotationPresent(Column.class)) {
					Column columnAnnotation = column
