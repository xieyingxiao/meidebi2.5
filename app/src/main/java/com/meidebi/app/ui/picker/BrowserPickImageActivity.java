...
/**
 * Created by mdb-ii on 15-1-28.
 */
public class BrowserPickImageActivity extends BasePullToRefreshActivity {
    public static final String FRAGMENT_INDEX = "FRAGMENT_INDEX";
    public static final String IMAGE_POSITION = "IMAGE_POSITION";
    public static final int INDEX = 2;
    protected List<AlbumUtil.PhotoEntry> imageUrls;
        ...
        switch (view.getId()) {
            case R.id.btn_pick_select_yes:
                if (XApplication.getInstance().getSeletedPhoto().size() == 0) {
                    Toast.makeText(BrowserPickImageActivity.this, "请选择至少一张图片", Toast.LENGTH_SHORT).show();
                } else {
    ...
    protected void initData() {
        imageUrls = (List<AlbumUtil.PhotoEntry>) getIntent().getSerializableExtra(PickerActivity.ALBUM_KEY);
    ...
    protected void initView(Bundle savedInstanceState) {
        ...
        btn_sure.setText("完成(" + XApplication.getInstance().getSeletedPhoto().size() + ")");
        btn_sure.setVisibility(View.VISIBLE);

        View title = getLayoutInflater()
                .inflate(R.layout.pick_browser_title_cb, null);
        cb_seleted = ((CheckBox) title.findViewById(R.id.cb_browser));
        cb_seleted.setChecked(AlbumUtil.isPicked(imageUrls.get(getIntent().getIntExtra(IMAGE_POSITION, 0))));
        cb_seleted.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                AlbumUtil.PhotoEntry current_photo = imageUrls.get(mViewPager.getCurrentItem());
                if (cb_seleted.isChecked()) {
                    if (XApplication.getInstance().getSeletedPhoto().get(current_photo.imageId) == null) {//同步加
                        PickerActivity.seleted_photos.add(current_photo);
                    }
                    XApplication.getInstance().getSeletedPhoto().put(current_photo.imageId, current_photo);
                } else {
                    if (XApplication.getInstance().getSeletedPhoto().get(current_photo.imageId) != null) {//同步减
                        PickerActivity.seleted_photos.remove(current_photo);
                    }
...
//                .showImageOnFail(R.drawable.ic_error)
        ...
        mViewPager.setOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                ...
                cb_seleted.setChecked(AlbumUtil.isPicked(imageUrls.get(position)));
            }
    ...
    private class ImageAdapter extends PagerAdapter {
        ...
        ImageAdapter() {
            inflater = LayoutInflater.from(BrowserPickImageActivity.this);
        }
        ...
        public Object instantiateItem(ViewGroup view, int position) {
            ...
            ImageLoader.getInstance().displayImage("file://" + imageUrls.get(position).path, imageView, options, new SimpleImageLoadingListener() {
                ...
                public void onLoadingFailed(String imageUri, View view, FailReason failReason) {
                    ...
                    Toast.makeText(BrowserPickImageActivity.this, message, Toast.LENGTH_SHORT).show();
