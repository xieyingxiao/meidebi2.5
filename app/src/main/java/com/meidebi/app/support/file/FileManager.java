...
//    private static final String PICTURE_THUMBNAIL = "pic_original";
...
//    private static final String WEBVIEW_FAVICON = "favicon";
    ...
    public static String getSdCardPath() {
        if (isExternalStorageMounted()) {
            File path = XApplication.getInstance().getExternalCacheDir();
            if (path != null)
                return path.getAbsolutePath();
            else {
                if (!cantReadBecauseOfAndroidBugPermissionProblem) {
                    cantReadBecauseOfAndroidBugPermissionProblem = true;
//                    XApplication.getInstance().getActivity().runOnUiThread(new Runnable() {
//                        @Override
//                        public void run() {
//                            new AlertDialog.Builder(XApplication.getInstance().getActivity())
...
//                    });
    ...
    public static String getFilePathFromUrl(String url, FileType method) {
...
//    }
    ...
    public static String getCacheSize() {
        if (isExternalStorageMounted()) {
            ...
            FileUtils size = new FileUtils(new File(path));
...
//            String middlePath = getSdCardPath() + File.separator + PICTURE_BMIDDLE;
            String oriPath = getSdCardPath() + File.separator;
...
//            size += new FileUtils(new File(thumbnailPath)).getLongSize();
//            size += new FileUtils(new File(middlePath)).getLongSize();
            size += new FileUtils(new File(oriPath)).getLongSize();

        }
        return FileUtils.convertSizeToString(size);
    }
